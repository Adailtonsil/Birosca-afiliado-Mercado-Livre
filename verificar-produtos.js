 ...
 * Retorna { ok: true, html, urlFinal } ou { ok: false, status, removido }.
 *
 * IMPORTANTE sobre o campo "removido":
 *   - true  => o link está confirmadamente quebrado/o produto não existe
 *              mais (404/410, ou a página carregou normalmente mas o
 *              próprio Mercado Livre exibe uma mensagem de que o anúncio
 *              foi pausado/encerrado/não está mais disponível).
 *   - false => falha temporária (erro de rede, timeout, bloqueio
 *              passageiro, erro 5xx do servidor). NÃO marcamos como
 *              removido nesses casos -- um problema técnico passageiro
 *              não significa que o produto deixou de existir, e marcar
 *              "indisponivel" por engano tiraria um produto válido do
 *              ar até a próxima verificação.
 */
async function buscarPaginaProduto(url) {
  try {
    ...
    // A página pode responder com status 200 (OK) mesmo quando o
    // anúncio não existe mais -- o Mercado Livre às vezes redireciona
    // para uma página genérica de "item não encontrado" ou mostra um
    // aviso de que o anúncio foi pausado/encerrado pelo vendedor, sem
    // usar um status HTTP de erro. Reconhecemos esses casos pelo texto
    // exibido na própria página.
    const indicaProdutoRemovido =
